                ...
                if minutos == 2 or minutos== 7:
                    try:
                        if i <7:
                            ...
                            cores = [
                                'verde' if velas[i-7+j]['open'] < velas[i-7+j]['close'] else
                                'vermelha' if velas[i-7+j]['open'] > velas[i-7+j]['close'] else
                                'Doji'
                                for j in range(7)
                            ]
                            #verificando se todos 7 velas são da mesma cor
                            if all(c == 'verde' for c in cores):
                                dir = 'vermelha' #se 7 verdes, entra na vermelha
                            elif all(c == 'vermelha' for c in cores):
                                dir = 'verde'
                            else: 
                                continue #se houver doji ou mistura não faz nada
                            ...
                            qnt_entrada += 1
                            dici_result = contabiliza_resultado(ativo, dir, entrada, entradamax, dici_result)
...
def puxa_candles():
    """Puxa as últimas 7 velas do par EURUSD-OTC e retorna uma lista de strings ('Verde', 'Vermelha', 'Doji')."""
