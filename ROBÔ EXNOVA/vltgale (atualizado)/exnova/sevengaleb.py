...
from exnovaapi.constants import ACTIVES
...
print("Iniciando Conexão com a Exnova")
...
def estrategia_LSTrader_Sevenflip(ativos, valor_entrada, tipo):
    ...
    while True:
        ...
        # Verificar se está no intervalo de entrada 
        #entrar = True if (minutos >= 1.58 and minutos <= 2.00) or (minutos >= 6.58 and minutos <= 7.00) else False 
                ...
                try:
                    ...
                    if volatilidade >= volatility_threshold:
                        ...
                        if volume_ultima >= min_volume:
                            direcao = "put" if velas[-1]["close"] > velas[-2]["close"] else "call"
                            if segundos < 30:
                                print(yellow + '***************************************************************************************')
                                print(f"\n>>>Padrão de Reversão identificado. Entrando na operação com {direcao} para o ativo {melhor_par}", h)
                                print(yellow + '***************************************************************************************')
                                print('\n')
                                puxa_candles()
                                ...
                                estrategia_LSTrader_Sevenflip(ativos, valor_entrada, tipo)                                                 
                        else:
...
def catag(all_asset,API):
    ...
    def SEVEN(vela1):
        ...
        for par in pares_abertos:
            ...
            calcula_resultado(par, 'SEVEN FLIP', dici_result)
...
def puxa_candles():
    ...
    print(f'>>> {green}PUXANDO ULTIMAS 7 VELAS DO PAR {white}{melhor_par}')
    
    velas = API.get_candles(f'{melhor_par}', 60, 7, time.time())
    hora0 = datetime.fromtimestamp(velas[-7]['from']).strftime('%H:%M')
    hora1 = datetime.fromtimestamp(velas[-6]['from']).strftime('%H:%M')
    hora2 = datetime.fromtimestamp(velas[-5]['from']).strftime('%H:%M')
    hora3 = datetime.fromtimestamp(velas[-4]['from']).strftime('%H:%M')
    hora4 = datetime.fromtimestamp(velas[-3]['from']).strftime('%H:%M')
    hora5 = datetime.fromtimestamp(velas[-2]['from']).strftime('%H:%M')
    hora6 = datetime.fromtimestamp(velas[-1]['from']).strftime('%H:%M')

    # Criar uma lista de strings representando as cores das velas
    cores = []
    cores.append('Verde' if velas[-7]['open'] < velas[-7]['close'] else 'Vermelha' if velas[-7]['open'] > velas[-7]['close'] else 'Doji')
    cores.append('Verde' if velas[-6]['open'] < velas[-6]['close'] else 'Vermelha' if velas[-6]['open'] > velas[-6]['close'] else 'Doji')
    ...
    print('\n')
    for _ in range(9):
        print('  ', cor_vela(cores[0]), ' ', cor_vela(cores[1]), ' ', cor_vela(cores[2]), ' ', cor_vela(cores[3]), ' ', cor_vela(cores[4]), ' ', cor_vela(cores[5]), ' ', cor_vela(cores[6]))

    print('   ', str(hora0), '   ', str(hora1), '   ', str(hora2), '   ', str(hora3), '   ', str(hora4), '   ', str(hora5), '   ', str(hora6))
    print('')

    return cores  # Retorna as cores das últimas 7 velas como uma lista de strings

def escolher_par_e_tipo_automatico(API, payout_minimo=70):
    global tipos_disponiveis, melhor_par, tempo_restante, ativo
    """ 
    Função para escolher automaticamente o par de moedas com o maior payout em 1 minuto,
    bloqueando os ativos que estiverem fechados ou OTC, e determinar o tipo de operação 
    ...
    ativos_excluidos = ["NZDUSD", "GBPJPY", "EURJPY", "USDJPY", "EURGBP_GS", "GBPUSD"]
    
    # Filtrar ativos que estejam abertos no timeframe de 1 minuto e tenham payout acima do mínimo 
    ativos_disponiveis = {} 
    pares_abertos = []

    timeframe = 1  # Timeframe de 1 minuto (60 segundos)

    # Verificar ativos digitais abertos em 1 minuto
    for par, info in all_assets['digital'].items():
        if info['open'] and info.get('expiration', 0) == timeframe and par not in ativos_excluidos:
            payout_digital = API.get_digital_payout(par)
            if payout_digital > 0 and payout_digital >= payout_minimo:
                ativos_disponiveis[par] = payout_digital
                pares_abertos.append({
                    'Ativo': par,
                    'Payout (%)': payout_digital,
                    'Tempo Restante (s)': info.get('expiration', 0)
                })

    # Verificar ativos binários abertos em 1 minuto
    for par, info in all_assets['binary'].items():
        if "OTC" not in par and info['open'] and par not in ativos_excluidos:
            payout_binario = all_profits.get(par, {}).get('binary', 0) * 100
            if payout_binario > 0 and payout_binario >= payout_minimo:
                ativos_disponiveis[par] = payout_binario
                pares_abertos.append({
                    'Ativo': par,
    ...
    # Se houver ativos disponíveis, mostrar as informações
    if ativos_disponiveis:
    
        print('>> Puxando pares abertos em 1 minuto', end='\r')
        # Coletar os payouts para exibir
        payouts = [[par, ativos_disponiveis[par]] for par in ativos_disponiveis]

        # Mostrar tabela de payouts
        print(tabulate(payouts, headers=['PAR', 'PAYOUT (%)'], tablefmt="double_grid", numalign="center"))
        
        
        # Mostrar catalogação dos pares disponíveis
        df_catalogacao = pd.DataFrame(pares_abertos)
        print("Catalogação dos pares disponíveis:")
        print(df_catalogacao)

        # Escolher o melhor par com maior payout
        melhor_par = max(ativos_disponiveis, key=ativos_disponiveis.get)
        print(yellow + '***************************************************************************************')
        print(f"Ativo escolhido: {melhor_par} com payout de {ativos_disponiveis[melhor_par]:.2f}%")
        print(yellow + '***************************************************************************************')

        
        # Verificar os tipos disponíveis
        tipos_disponiveis = {
        ...
        # Filtrar os tipos com payout acima do mínimo
        tipos_validos = {k: v for k, v in tipos_disponiveis.items() if v >= payout_minimo}
        # Escolher o melhor tipo de negociação disponível
        if tipos_validos:
            melhor_tipo = max(tipos_validos, key=tipos_validos.get)
            print(f"Tipo de negociação: {melhor_tipo.upper()} com payout de {tipos_validos[melhor_tipo]:.2f}%")
            return melhor_par, melhor_tipo
        else:
            print(f"Nenhum tipo de negociação disponível para {melhor_par} com payout mínimo de {payout_minimo}%.")
            return melhor_par, None
    else:
        print(f"Nenhum ativo disponível com payout mínimo de {payout_minimo}%.")
        return None, None

def wait_for_new_candle(API):
    """
    Função que aguarda o momento exato para o início de um novo candle.
    O robô aguardará até o início exato do próximo candle.
    """
    while True:
        # Obtém o horário atual da corretora
        now = get_broker_time(API)

        # Verifica se o próximo candle está a menos de 1 segundo de começar (novo candle sempre no segundo 0)
        # Exibe o horário da corretora enquanto aguarda o minuto certo (minuto 4 em ciclo de 5 minutos)
        
        #if now.minute % 5 == 4 and now.second == 00:
        if now.second == 58:
            print(f"\n>>>{yellow}Processo iniciado... {blue}Aguardando o próximo candle.")

            # Calcula o tempo até o próximo candle (minuto seguinte, segundo 0)
            next_candle = (now + timedelta(seconds=1)).replace(microsecond=0)

            # Aguardar até o momento exato de virada do candle
            while get_broker_time(API) < next_candle:
                print(f"\r>>>{green}Aguardando o minuto certo. {cyan}Horário da corretora: {white}{now.strftime('%H:%M:%S')}", end="")
                time.sleep(0.05)  # Verifica a cada 50ms para garantir precisão
            print(f"\n>>>{blue}Novo candle iniciado! {green}Entrando na operação.")
            break

        # Aguarda até o próximo segundo para verificar novamente
        next_check_time = (now + timedelta(seconds=1)).replace(microsecond=0)
        while get_broker_time(API) < next_check_time:
            print(f"\r>>>{green}Aguardando o minuto certo. {cyan}Horário da corretora: {white}{now.strftime('%H:%M:%S')}", end="")
            time.sleep(0.05)  # Aguarda com alta precisão até o próximo segundo

        time.sleep(0.05)  # Aguarda brevemente para evitar consumo excessivo de CPU
        
        # Atualiza o tempo restante enquanto aguarda o minuto 4 do ciclo de 5 minutos
        next_minute_check = (now + timedelta(seconds=1)).replace(microsecond=0)
        while get_broker_time(API) < next_minute_check:
            now = get_broker_time(API)
            remaining_time = next_minute_check - now
            minutes, seconds = divmod(remaining_time.total_seconds(), 58)
            print(f"\r>>>{cyan}Tempo restante até o próximo candle: {int(minutes):02}:{int(seconds):02}", end="")
            time.sleep(0.05)

        time.sleep(0.05)  # Evita consumo excessivo de CPU
